...
export default class JSONPointer {
  ...
  private parseParts(str: string, path: TDPath, relativeWithNum: boolean): boolean {
    ...
    if (relativeWithNum) {
      ...
      if (!parts[0]) path.addParts(Part.ofRoot());
      else if ('.' === parts[0]) path.addParts(Part.ofRelative(0));
      else if ('..' === parts[0]) path.addParts(Part.ofRelative(1));
      else path.addParts(Part.ofId(parts[0]));
    }
